...
class Game:
    ...
    def RayCast(self):
        ...
        for i in range(0, r):
            ...
            stopNext = False
            for j in range(0, 20):
                x = self.Player.x + (self.Player.size / 2) + (math.cos(math.radians(i *(360 / r))) * (20 * j))
                y = self.Player.y + (self.Player.size / 2) + (math.sin(math.radians(i *(360 / r))) * (20 * j))
...
class Grid:
    ...
    def Setup(self):
        #Each tile should switch between 2 colors
        color = grasscolor[0]
